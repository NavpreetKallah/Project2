...
class Hud:
    def __init__(self):
        ...
        self.spritesheet = pygame.transform.scale_by(pygame.image.load_extended(f"{path}/numbers.png").convert_alpha(),
                                                     SCALE)
        self.fast_forward_indicator = pygame.transform.scale_by(pygame.image.load_extended(f"{path}/fast_forward.png").convert_alpha(),
                                                   SCALE)
    ...
    def tower_chosen(self):
        ...
        for tower, info in self.tower_dict.items():
            if not info["locked"] and info["rect"].collidepoint((mouse[0] - 129 * SCALE, mouse[1] - 12 * SCALE)):
